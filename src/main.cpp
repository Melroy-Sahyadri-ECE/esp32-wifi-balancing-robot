...
void setup() {
  ...
  // Configure static IP (matching your network)
  IPAddress local_IP(10, 245, 242, 100);  // Static IP address
  IPAddress gateway(10, 245, 242, 1);     // Router IP
  ...
  if (WiFi.status() == WL_CONNECTED) {
    ...
    Serial.println("");
    Serial.println("❌ WiFi connection failed to " + sta_ssid);
    ...
    if (WiFi.status() == WL_CONNECTED) {
      ...
      Serial.println("❌ WiFi connection failed completely");
  ...
  server.on("/", HTTP_GET, [] (AsyncWebServerRequest *request) {
    ...
    html += "<h1>🤖 Balancing Robot Control</h1>";
    ...
    html += "<div class='value-display' id='servo1Value'>90°</div>";
    html += "</div>";
    html += "<div class='control-group'>";
    ...
    html += "<div class='value-display' id='servo2Value'>90°</div>";
    html += "</div>";
    html += "<div class='control-group'>";
    ...
    html += "servo1Value.textContent = angle + '°';";
    html += "fetch('/control?servo1=' + angle)";
    html += ".then(function(response) { return response.text(); })";
    html += ".then(function(data) { document.getElementById('status').innerHTML = 'Servo 1: ' + angle + '°'; })";
    html += ".catch(function(error) { document.getElementById('status').innerHTML = 'Error: ' + error; });";
    html += "}";
    html += "function updateServo2() {";
    html += "var angle = servo2Slider.value;";
    html += "servo2Value.textContent = angle + '°';";
    html += "fetch('/control?servo2=' + angle)";
    html += ".then(function(response) { return response.text(); })";
    html += ".then(function(data) { document.getElementById('status').innerHTML = 'Servo 2: ' + angle + '°'; })";
    html += ".catch(function(error) { document.getElementById('status').innerHTML = 'Error: ' + error; });";
    html += "}";
...
void motorControl() {
  // Only run motors if angle is reasonable (within ±45 degrees)
...
int validateAngle(int angle) {
  if (angle < 0) {
    Serial.print("Angle ");
    Serial.print(angle);
    Serial.print("° clamped to minimum 0°");
    Serial.println();
    return 0;
  } else if (angle > 180) {
    Serial.print("Angle ");
    Serial.print(angle);
    Serial.print("° clamped to maximum 180°");
    Serial.println();
    return 180;
  }
  return angle;
