...
{
  // Calculate accelerometer angle (tilt angle from vertical)
  accel_angle = atan2f((float)accel_t_gyro.value.y_accel, (float)accel_t_gyro.value.z_accel) * RAD2GRAD;
  
  // Convert gyro reading to degrees per second (500°/s full scale = 65.5 LSB/°/s)
  ...
  // Higher gyro weight (0.98) for better dynamic response
  angle = 0.98 * (angle + x_gyro_value * dt) + 0.02 * accel_angle;

  // Gyro bias correction - slowly adjust offset to compensate for drift
  if (abs(x_gyro_value) < 1.0) { // Only correct when robot is relatively stable
    int16_t correction = constrain(accel_t_gyro.value.x_gyro, x_gyro_offset - 5, x_gyro_offset + 5);
    x_gyro_offset = x_gyro_offset * 0.9998 + correction * 0.0002; // Slower correction
  }
...
int MPU6050_read(int start, uint8_t *buffer, int size)
{
  ...
  Wire.requestFrom((uint8_t)MPU6050_I2C_ADDRESS, (size_t)size, true);
  i = 0;
